...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [recent, setRecent] = useState<string[]>(() => {
    try {
      const raw = typeof window !== "undefined" ? localStorage.getItem("cn-recent-searches") : null
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault()
      setActiveRaw((a) => (a + 1) % flat.length)
    } else if (e.key === "ArrowUp") {
      e.preventDefault()
      setActiveRaw((a) => (a - 1 + flat.length) % flat.length)
    } else if (e.key === "Enter") {
      e.preventDefault()
                ...
                <div key={g.name} className="mb-1">
                  ...
                  {g.items.map(({ item, index }) => (
                    <button
                      key={item.key}
                      data-idx={index}
                      onMouseMove={() => setActiveRaw(index)}
